...
export default function CSVDataTable({ csvUrl }: CSVDataTableProps) {
  ...
  const fallbackData = [
    ...
    [
      ...
      "Computer Science",
      "Fourth Year",
      "Help international students adapt to campus life and connect with cultural communities",
      "Traditional dance, Cooking, Photography",
    ],
    [
      ...
      "Third Year",
      "Share career networking tips and support first-gen college students",
      "Cricket, Music production, Travel",
    ],
    [
      ...
      "Engineering",
      "Graduate Student",
      "Guide undergrads in research opportunities and cultural identity exploration",
      "Martial arts, Gaming, Language exchange",
    ],
  ];
  ...
  const fetchCSVData = useCallback(async () => {
    try {
      ...
      if (errorMessage.includes("404")) {
        ...
        setError(
          `Unable to load live data. This may be due to Google Sheets permissions or network restrictions.`
        );
      ...
      <div className="py-8 text-center">
        <div className="mx-auto max-w-md rounded-lg border border-amber-200 bg-amber-50 p-6">
          ...
          <p className="mb-4 text-sm text-amber-700">
            Unable to load live responses from Google Sheets. This may be due to permission settings
          ...
          <div className="flex flex-col gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg bg-amber-100 px-4 py-2 text-sm font-medium text-amber-700 transition-colors hover:bg-amber-200"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg bg-[#8bd4e0] px-4 py-2 text-sm font-medium text-gray-800 transition-colors hover:bg-[#7bc7d3]"
            >
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  strokeLinecap="round"
  ...
  return (
    <div className="overflow-x-auto">
      {showFallback && (
        <div className="mb-4 rounded-xl border border-[#8bd4e0]/30 bg-[#8bd4e0]/10 p-4">
          <div className="flex items-start gap-3 text-gray-700">
            <svg className="mt-0.5 h-5 w-5 text-[#8bd4e0]" fill="currentColor" viewBox="0 0 20 20">
              <path
                fillRule="evenodd"
            ...
            <div>
              <div className="mb-1 font-medium text-gray-900">Sample Data Display</div>
              <div className="text-sm text-gray-600">
                Live mentor signup data is not currently accessible. The table below shows example
                entries to demonstrate the format.
              </div>
            </div>
          </div>
        </div>
        ...
        <table className="w-full rounded-lg border border-gray-200 bg-white shadow-sm">
          ...
          <tbody>
            {rows.map((row, rowIndex) => (
              <tr
                key={rowIndex}
                className={`border-b border-gray-100 ${rowIndex % 2 === 0 ? "bg-white" : "bg-gray-50/50"} transition-colors hover:bg-gray-50 ${showFallback ? "opacity-75" : ""}`}
              >
                {headers.map((_, cellIndex) => (
                  <td key={cellIndex} className="whitespace-nowrap px-4 py-3 text-sm text-gray-700">
                    {row[cellIndex] || ""}
                  </td>
                ))}
              </tr>

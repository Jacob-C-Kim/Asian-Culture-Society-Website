...
export default function CalendarWidget({
  ...
  return (
    <div
      className="w-full max-w-none rounded-3xl bg-white shadow-lg transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)]"
      style={{
    ...
    >
      <Calendar
        ...
        className="calendar-with-events w-full rounded-md border-0 transition-all duration-300 ease-out"
        modifiers={{
          hasEvent: eventDates,
        }}
        modifiersClassNames={{
        ...
        classNames={{
          ...
          nav_button: `bg-transparent opacity-70 hover:opacity-100 border-0 rounded-lg hover:bg-gray-50 transition-all duration-200 flex items-center justify-center ${
            isMobile ? "w-10 h-10 p-2 active:scale-95" : isCompact ? "w-8 h-8 p-3" : "w-10 h-10 p-3"
          }`,
          ...
          day: `w-full font-medium hover:bg-gray-100 transition-all duration-200 mx-auto flex items-center justify-center cursor-pointer rounded-xl border-0 ${
            isMobile
              ...
              : isCompact
                ? "h-10 text-base hover:scale-105"
                : "h-16 text-xl hover:scale-105"
          }`,
          day_selected:
            "bg-blue-500 text-white hover:bg-blue-600 focus:bg-blue-600 rounded-xl transform scale-105",
